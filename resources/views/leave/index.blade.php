...
@section('content')
            ...
            <div class="row p-4">
                <div class="container">
                    <div class="email-header">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                ...
                                @else
                                    <p class="mb-0 opacity-75">Check your leave status then go to home and enjoy your
                                        precious movements of life.</p>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="container-fluid p-4 border shadow-sm rounded bg-white ">
                            ...
                            <div>
                                @if (Auth::user()->role === 'team_member' || Auth::user()->role === 'team_leader' || Auth::user()->role === 'hr')
                                    <div class="mb-3 float-end">
                                        <a href="{{ route('create.leave') }}"><button class="btn btn-primary">Add
                                                Leave</button></a>
                                    </div>
                                @endif
                            </div>
                        ...
                        <form id="bulkDeleteForm" method="POST" action="{{ route('leaves.bulk.delete') }}"
                            style="display:none;">
                            @csrf
                        </form>
                            ...
                            <tbody>
                                @foreach ($leaves as $index => $item)
                                    <tr>
                                        ...
                                        @endif
                                        <td>{{ $index + 1 }}</td>
                                        <td><b>{{ $item->user->full_name }}</b>
                                            ({{ strtoupper(str_replace('_', ' ', $item->user->position)) }})
                                        </td>
                                        ...
                                        @if (
                                            $item->user->role === 'team_leader' &&
                                                (Auth::user()->role === 'hr' || Auth::user()->role === 'manager' || Auth::user()->id === $item->user_id))
                                            <td>--</td>
                                        @else
                                            <td>{{ ucfirst($item->team_leader_status) }}</td>
                                        @endif
                                        @if (
                                            (Auth::user()->role === 'manager' || Auth::user()->role === 'hr' || Auth::user()->role === 'team_leader') &&
                                                Auth::user()->id != $item->user_id)
                                            <td>
                                                {{-- Manager/HR status update form --}}
                                                @if (in_array(Auth::user()->role, ['manager', 'hr']))
                                                    <form id="statusForm-{{ $item->id }}"
                                                        action="{{ route('leave.updateStatus', $item->id) }}"
                                                ...
                                                @if (Auth::user()->role === 'team_leader')
                                                    <form id="teamLeaderForm-{{ $item->id }}"
                                                        action="{{ route('leave.updateTeamLeaderStatus', $item->id) }}"
                                                        method="POST" style="display:inline-block;">
                                                        @csrf
                                                        @method('PUT')
                                                        <input type="hidden" name="reject_reason"
                                                            id="tlRejectReason-{{ $item->id }}">
                                                        <select name="team_leader_status"
                                                            class="form-select form-select-sm teamleader-select mt-2"
                                                            data-form-id="teamLeaderForm-{{ $item->id }}"
                                                            data-current-value="{{ $item->team_leader_status }}"
                                                            data-item-id="{{ $item->id }}">
                                                            <option value="pending"
                                                                {{ $item->team_leader_status === 'pending' ? 'selected' : '' }}>
                                                                Pending</option>
                                                            <option value="accepted"
                                                                {{ $item->team_leader_status === 'accepted' ? 'selected' : '' }}>
                                                                Accepted</option>
                                                            <option value="rejected"
                                                                {{ $item->team_leader_status === 'rejected' ? 'selected' : '' }}>
                                                                Rejected</option>
                                                        </select>
                                                    </form>
                                                @endif
                                                ...
                                                @if (in_array(Auth::user()->role, ['manager', 'hr']))
                                                    <form action="{{ route('delete.leave', $item->id) }}" method="POST"
                                                        ...
                                                        <button class="btn btn-sm btn-danger mt-2"
                                                            onclick="return confirm('Delete this leave?')">Delete</button>
                                                    </form>
                                                @endif
                                            </td>
                                        ...
                                        @if (Auth::user()->id == $item->user_id)
                                            <td>
                                                @if ($item->team_leader_status === 'pending')
                                        ...
                                        <td>
                                            <!-- VIEW BUTTON -->
                                            <button class="btn btn-sm btn-info text-white viewLeaveBtn"
                                                data-id="{{ $item->id }}" data-name="{{ $item->user->full_name }}"
                                                data-position="{{ $item->user->position }}"
                                                data-from="{{ \Carbon\Carbon::parse($item->date_from)->format('d-M-Y') }}"
                                                data-to="{{ \Carbon\Carbon::parse($item->date_to)->format('d-M-Y') }}"
                                                data-type="{{ $item->leave_type }}" data-reason="{{ $item->reason }}"
                                                data-status="{{ ucfirst($item->status) }}"
                                                data-tl="{{ ucfirst($item->team_leader_status) }}"
                                                data-reject="{{ $item->reject_reason ?? '-' }}">
    ...
    <div class="modal fade" id="viewLeaveModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content shadow-lg border-0" style="border-radius: 18px; overflow:hidden;">
                ...
                <!-- Header -->
                <div class="modal-header" style="background: linear-gradient(135deg, #4f46e5, #2563eb); color:white;">
                    <h5 class="modal-title">
                        <i class="fa-solid fa-eye me-2"></i> Leave Details
                    </h5>
                ...
                <div class="modal-body p-4">
                    ...
                    <!-- Employee Box -->
                    <div class="d-flex align-items-center mb-4 p-3" style="background:#f8faff; border-radius: 12px;">
                        <div class="me-3">
                            <i class="fa-solid fa-user-circle" style="font-size:42px; color:#4f46e5;"></i>
                        </div>
                        <div>
                            <h5 class="mb-1 fw-bold" id="v_name"></h5>
                            <small class="text-muted" id="v_position"></small>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                <label class="text-muted small mb-1">Date From</label>
                                <div class="fw-semibold" id="v_from"></div>
                            </div>
                        </div>

                        <div class="col-md-6">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                <label class="text-muted small mb-1">Date To</label>
                                <div class="fw-semibold" id="v_to"></div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                ...
                                <div class="fw-semibold" id="v_type"></div>
                            </div>
                        </div>

                        <div class="col-md-6">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                <label class="text-muted small mb-1">Manager/HR Status</label>
                                <div id="v_status" class="fw-semibold"></div>
                            </div>
                        </div>

                        <div class="col-md-6">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                <label class="text-muted small mb-1">Team Leader Status</label>
                                <div id="v_tl" class="fw-semibold"></div>
                            </div>
                        </div>
                        ...
                        <div class="col-6">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                <label class="text-muted small mb-1">Reason</label>
                                <div id="v_reason" class="fw-semibold"></div>
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="p-3 border rounded shadow-sm bg-white">
                                <label class="text-muted small mb-1">Reject Reason</label>
                                <div id="v_reject" class="fw-semibold text-danger"></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            bulkDeleteBtn.addEventListener('click', function(e) {
                ...
                if (checkedBoxes.length === 0) {
                    Swal.fire('No Leave Selected', 'Please select at least one leave to delete.',
                        'warning');
                    return;
                }
                Swal.fire({
                    ...
                    bulkDeleteForm.querySelectorAll('input[name="leave_ids[]"]').forEach(n => n
                        .remove());
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // VIEW Leave Button Click
            document.querySelectorAll('.viewLeaveBtn').forEach(btn => {
                btn.addEventListener('click', function() {

                    document.getElementById('v_name').innerText = this.dataset.name;
                    document.getElementById('v_position').innerText =
                        this.dataset.position.replace(/_/g, " ").toUpperCase();

                    document.getElementById('v_from').innerText = this.dataset.from;
                    document.getElementById('v_to').innerText = this.dataset.to;
                    document.getElementById('v_type').innerText = this.dataset.type;
                    document.getElementById('v_reason').innerText = this.dataset.reason;
                    document.getElementById('v_status').innerText = this.dataset.status;
                    document.getElementById('v_tl').innerText = this.dataset.tl;
                    document.getElementById('v_reject').innerText = this.dataset.reject;
